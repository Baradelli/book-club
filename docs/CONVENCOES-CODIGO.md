...
| `ILIKE` do `text` (Tarefas 10 e 29) | Case-insensitive **sim**, accent-insensitive **NÃO**: `'coração' ILIKE '%coracao%'` é falso — reconferido no Postgres na Tarefa 29, com `'CORAÇÃO' ILIKE '%coração%'` = **true** como par positivo | Passaria verde e a busca real não acharia nada. ⚠️ **Esta linha previa que o `unaccent` seria a Tarefa 29, "com migration e ADR". NÃO FOI:** a 29 mediu que ligar a extensão exige DDL no banco do dono + índice funcional + ADR, deixou o `unaccent` **fora** de escopo e o transformou em **pergunta do dono** no `docs/ACEITE-MVP.md`. A fidelidade que a 29 de fato trouxe foi o `matchesText` de **duas colunas** (`quote` **e** `commentText`) — e ela nasceu **já** no arquivo compartilhado, que é o que a 4ª aparição abaixo existia para comprar. Previsão de fatia futura numa tabela de medições é a única entrada deste documento que envelheceu sozinha; ficou registrada em vez de apagada, porque o erro é a lição |
